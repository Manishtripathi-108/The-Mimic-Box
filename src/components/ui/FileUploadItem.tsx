...
const FileUploadItem = ({ file, error, onRemove, children, className }: FileUploadItemProps) => {
    ...
    return (
        <section
            className={cn(
                'shadow-pressed-xs relative mb-2 flex w-full flex-col items-center justify-between gap-4 rounded-xl border p-4 sm:flex-row',
                className
            )}
            ...
            <button
                ...
                onClick={onRemove}
                className="sm:button absolute top-4 right-3 size-5 cursor-pointer text-red-500 sm:relative sm:top-0 sm:right-0 sm:size-9 sm:rounded-xl sm:p-1 dark:text-red-500">
